...
export function Tabs({ notes, currentNoteId, onAdd, onSwitch, onRename, onDelete }: Props) {
  ...
  const openMenuAt = (id: string, x: number, y: number) => {
    const now = Date.now();
    if (now - lastOpenRef.current < 150) return; // 중복 이벤트 디바운스
  ...
  useEffect(() =>
  {
    ...
    const onCtx = (e: MouseEvent) =>
    {
      ...
      const tabBtn = target?.closest?.('button[data-id]') as HTMLButtonElement | null;
      if (!tabBtn)
      {
        return;
      }
  ...
  useEffect(() => {
    const onDocClick = (e: MouseEvent) => {
      if (!menuOpen) return;
      if (menuRef.current && menuRef.current.contains(e.target as any)) return; // 메뉴 내부 클릭은 무시
      setMenuOpen(false);
    };
  ...
  return (
    ...
    >
        ...
        <div key={n.id} style={{ display: 'inline-block' }}>
          {editingId === n.id ? (
            ...
            <input
              ...
              style={{ width: Math.max(60, ((tempName || n.name).length + 2) * 8) }}
              autoFocus
            />
          ) : (
      ...
      {!maxReached && (
        ...
        <button id="addNoteBtn" className="toolbar-btn" title="Add Note" onClick={onAdd}>+</button>
      )}
      ...
      {menuOpen && (
        ...
        <div
          ...
          style={{ position: 'fixed', left: menuPos.x, top: menuPos.y, display: 'block' }}
