...
export function useNotes(vscode: VSCodeApi)
{
  ...
  const switchNote = (id: string) => vscode.postMessage({ command: 'switchNote', id });
  const renameNote = (id: string, name: string) => vscode.postMessage({ command: 'renameNote', id, name });
  const deleteNote = (id: string) => vscode.postMessage({ command: 'deleteNote', id });
  const resetAllMemos = () => vscode.postMessage({ command: 'resetAllMemos' });
