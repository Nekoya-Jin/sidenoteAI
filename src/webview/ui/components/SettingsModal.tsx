/*
  SettingsModal 컴포넌트
  - 역할: 설정(UI) 모달 표시, Gemini API Key 입력/저장/초기화 처리 + 모든 노트 초기화 버튼 제공(테스트/관리용)
  - 핵심 포인트:
    1) 닫기 버튼은 우측 상단의 X 버튼으로 제공
    2) Save 클릭 시 저장만 수행하고, 모달은 닫지 않음(저장 완료 후 설정 재조회)
    3) Reset All Notes는 별도 섹션(가운데 정렬)
*/
...
type Props = {
  ...
  onSave: () => void;
  onResetAllNotes: () => void; // 모든 노트 초기화
};
    ...
    <div className="modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div>
          ...
          >
            <button className="btn stable" onClick={onSave} disabled={saving}>
              Save
            </button>
          </div>
