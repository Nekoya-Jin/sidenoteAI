...
export class NotesService
{
  ...
  public getSnapshot()
  {
    return { notes: this.notesList, currentNoteId: this.currentNoteIdValue, content: this.currentContentValue };
  }
  ...
  public createNote(desiredName?: string): Note
  {
    ...
    const base = (desiredName ?? 'memo').trim() || 'memo';
  ...
  public renameNote(id: string, newNameRaw: string)
  {
    ...
    const conflict = this.notesList.some(m => m.id !== id && m.name.toLowerCase() === lower);
    if (conflict)
    {
  ...
  public deleteNote(id: string)
  {
    ...
    {
      this.currentNoteIdValue = this.notesList.length > 0 ? this.notesList[0].id : '';
      this.currentContentValue = this.currentNoteIdValue ? this.getNoteContent(this.currentNoteIdValue) : '';
    }
    this.saveNotes();
  }
  ...
  public async resetAll(): Promise<void>
  {
    ...
    const contentPrefix = `memo_${base}_`;
    ...
    if (Array.isArray(keys) && keys.length)
    {
      for (const k of keys)
      {
        if (
          ...
          k === currentKey ||
          k === legacyKey ||
          (typeof k === 'string' && k.startsWith(contentPrefix))
        )
        {
  ...
  {
    return `memo_${this.baseKey()}_${id}`;
  }

  private loadNotes()
  {
    const notes = this.context.workspaceState.get<Note[]>(this.notesKey(), []);
    const current = this.context.workspaceState.get<string>(this.currentKey(), '');
    ...
    if (!notes || notes.length === 0)
    {
      ...
      const legacy = this.context.workspaceState.get<string>(legacyKey, this.defaultTemplate());
      const id = 'note-1';
      const name = this.ensureUniqueName('memo');
  ...
  private saveNotes()
  {
    ...
    this.context.workspaceState.update(this.currentKey(), this.currentNoteIdValue);
  }
  ...
  {
    const base = (desiredBase || 'memo').trim();
  ...
  private defaultTemplate(): string
  {
    ...
    return [
      ...
      '  - Nested item 2',
      '',
      '> Tip: Right-click a tab to Rename or Delete. Use the toolbar to add a new memo.',
      '',
      '## Code',
      '```ts',
