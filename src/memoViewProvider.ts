...
export class MemoViewProvider implements vscode.WebviewViewProvider {
    ...
    constructor(context: vscode.ExtensionContext) {
        ...
        this._settings = new SettingsService(context, MemoViewProvider.DEFAULT_PROMPT);
    ...
    public resolveWebviewView(
        ...
        webviewView.webview.html = buildHtml(this._context.extensionUri, webviewView.webview);

        // Register message handlers
        this._router.on('updateMemo', (m) => {
            this._notesService.updateCurrentContent(m.content ?? '');
            this.sendCurrentNoteContent();
        });
        this._router.on('getMemo', () => {
            this.sendCurrentNoteContent();
        });
        ...
        this._router.on('requestSummary', async (m) => {
            ...
            try {
                ...
                webviewView.webview.postMessage({ command: 'summaryResult', text: result });
            } catch (err: any) {
                const msg = err?.message ?? String(err);
                webviewView.webview.postMessage({ command: 'summaryError', message: msg });
            }
        });
        ...
        this._router.on('renameNote', (m) => {
            try {
                ...
                if (e?.message === 'NAME_CONFLICT') {
                    vscode.window.showErrorMessage('A note with the same name already exists. Please choose a different name.');
                } else {
                    vscode.window.showErrorMessage(String(e?.message ?? e));
                }
            }
        ...
        this._router.on('resetAllMemos', async () => {
            ...
            this.sendNotesData();
            this.sendCurrentNoteContent();
            vscode.window.showInformationMessage('All memos have been reset to defaults.');
        });

        webviewView.webview.onDidReceiveMessage(
            async (message) => { await this._router.handle(message); },
            undefined,
    ...
    public clearMemo() {
        vscode.window
            .showWarningMessage('Are you sure you want to clear the memo?', 'Yes', 'No')
            .then((selection) => {
                if (selection === 'Yes') {
                    this._notesService.clearCurrentNote();
                    this._view?.webview.postMessage({ command: 'loadMemo', content: this._notesService.currentContent });
                    vscode.window.showInformationMessage('Memo cleared!');
                }
            });
    }
    ...
    private sendNotesData() {
        this._view?.webview.postMessage({ command: 'notesData', notes: this._notesService.notes, currentNoteId: this._notesService.currentNoteId });
    }
